...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await ProjectMediaModel.bulkWrite(operations);
    const projects = await ProjectMediaModel.find().sort({ order: 1 });
    return NextResponse.json({ data: projects, message: 'Order updated' });
  } catch (error: any) {
